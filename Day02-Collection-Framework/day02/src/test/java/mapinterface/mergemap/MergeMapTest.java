...
import mapinterface.mergetwomaps.MergeMaps;
...
public class MergeMapTest {
        ...
        void testMergeMap(){
            ...
            // Merge the two maps
            Map<Character, Integer> actualMap = MergeMaps.mergeMap(map1, map2);

            // Expected merged map
            Map<Character, Integer> output = new HashMap<>(Map.of('A', 1, 'B', 5, 'C', 4));

            // Assert that the actual merged map matches the expected map
            Assertions.assertEquals(output, actualMap);
        }
    }
